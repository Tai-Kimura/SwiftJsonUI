...
# sjui_toolsディレクトリへのパス設定
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SJUI_TOOLS_DIR="$(dirname "$SCRIPT_DIR")"
PROJECT_ROOT_DIR="$(dirname "$SJUI_TOOLS_DIR")"

echo "SCRIPT_DIR: $SCRIPT_DIR"
echo "SJUI_TOOLS_DIR: $SJUI_TOOLS_DIR"
echo "PROJECT_ROOT_DIR: $PROJECT_ROOT_DIR"

# configファイルからsource_directoryを取得
get_source_directory() {
    local config_file="$SJUI_TOOLS_DIR/config.json"
    local source_dir=""
...
echo "SJUI Tools Dir: $SJUI_TOOLS_DIR"
