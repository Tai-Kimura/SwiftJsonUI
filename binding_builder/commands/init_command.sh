...
EOF
    ...
    if [ $? -ne 0 ]; then
        ...
        if [ -f "$BINDING_BUILDER_PARENT/App.swift" ] || [ -f "$BINDING_BUILDER_PARENT/ContentView.swift" ] || [ -f "$BINDING_BUILDER_PARENT/AppDelegate.swift" ] || [ -f "$BINDING_BUILDER_PARENT/SceneDelegate.swift" ] || [ -f "$BINDING_BUILDER_PARENT/Info.plist" ]; then
            ...
            for dir in "$BINDING_BUILDER_PARENT"/*; do
                if [ -d "$dir" ] && [ "$(basename "$dir")" != "binding_builder" ] && { [ -f "$dir/App.swift" ] || [ -f "$dir/ContentView.swift" ] || [ -f "$dir/AppDelegate.swift" ] || [ -f "$dir/SceneDelegate.swift" ] || [ -f "$dir/Info.plist" ]; }; then
