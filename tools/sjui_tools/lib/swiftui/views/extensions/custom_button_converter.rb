...
module SjuiTools
  module SwiftUI
    module Views
      module Extensions
        class CustomButtonConverter < BaseViewConverter
          ...
          def convert
            ...
            params = []
            if @component['text']
              formatted_value = format_value(@component['text'], 'String')
              params << "text: #{formatted_value}" if formatted_value
            end
            if @component['action']
              formatted_value = format_value(@component['action'], 'String')
              params << "action: #{formatted_value}" if formatted_value
            end
            if @component.key?('isEnabled')
              formatted_value = format_value(@component['isEnabled'], 'Bool')
              params << "isEnabled: #{formatted_value}" if formatted_value != nil
            end
            
            # Non-container component
            if params.empty?
              add_line "CustomButton()"
            else
              add_line "CustomButton("
              indent do
