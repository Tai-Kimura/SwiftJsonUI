...
public struct PartialAttributedText: View {
    ...
    private func createAttributedStringWithMapping() -> (attributedString: AttributedString, urlMapping: [String: PartialAttribute]) {
        ...
        // fontFamily takes priority: use fontProvider -> Font.custom() -> system font
        if let fontFamily = fontFamily {
            let size = fontSize ?? SwiftJsonUIConfiguration.shared.font.size
            if let providedFont = SwiftJsonUIConfiguration.shared.fontProvider?(fontFamily) {
                attributedString.font = providedFont
            } else {
                attributedString.font = .custom(fontFamily, size: size)
            }
...
struct BaseFontModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        if let fontFamily = fontFamily {
            // fontFamily specified: use fontProvider -> Font.custom()
            let size = fontSize ?? SwiftJsonUIConfiguration.shared.font.size
            if let providedFont = SwiftJsonUIConfiguration.shared.fontProvider?(fontFamily) {
                content.font(providedFont)
            } else {
                content.font(.custom(fontFamily, size: size))
            }
        } else if let fontSize = fontSize, let fontWeight = fontWeight {
