...
public class StyleProcessor {
    ...
    // Use same cache path as HotLoader (SJUIViewCreator.getStyleFileDirPath())
    private static var cacheDirPath: String {
        let paths = NSSearchPathForDirectoriesInDomains(.cachesDirectory, .userDomainMask, true)
        return paths[0] + "/Styles/"
    }
    #endif
    ...
    public static func processStyles(_ json: [String: Any]) -> [String: Any] {
        ...
        if let styleName = json["style"] as? String {
            Logger.debug("[StyleProcessor] Loading style: '\(styleName)'")
            if let styleData = loadStyle(named: styleName) {
                ...
                Logger.debug("[StyleProcessor] Successfully applied style '\(styleName)' to component")
            } else {
                Logger.debug("[StyleProcessor] Failed to load style '\(styleName)'")
            }
        }
    ...
    private static func loadStyle(named name: String) -> [String: Any]? {
        ...
        Logger.debug("[StyleProcessor] Checking cache path: \(cacheFilePath)")
        ...
        if let resourcePath = Bundle.main.resourcePath {
            do {
                ...
                Logger.debug("[StyleProcessor] All JSON files in bundle: \(jsonFiles)")
