...
public class SwiftJsonUIConfiguration: ObservableObject {
    ...
    ///
    /// Receives a fully-populated ``FontSpec`` describing the JSON-level font
    /// attributes (family / weight / size / italic) and returns a resolved
    /// `Font`. Return `nil` to fall through to the library's built-in default
    /// resolution (see ``resolveFont(_:)``).
    ///
    /// **Breaking change in 9.5.0**: previously typed
    /// `((String) -> Font?)?` where the string carried *either* a weight name
    /// *or* a family name depending on the call site. The new signature passes
    /// every font-related attribute together so apps can implement one rule
    /// covering the full matrix of weight × family × size × italic.
    public var fontProvider: ((FontSpec) -> Font?)? = nil
    ...
    // MARK: - Font Resolution (9.5.0+)

    /// Resolve a `Font` from a ``FontSpec``.
    ///
    /// 1. If ``fontProvider`` is set and returns non-nil, that wins.
    /// 2. Otherwise the library builds a sensible default:
    ///    - `family != nil` → `Font.custom(family, size: size ?? 17)`,
    ///      with `.weight(weight)` applied when present.
    ///    - `family == nil` → `Font.system(size: size ?? 17, weight: weight ?? .regular)`.
    ///    - `italic == true` → `.italic()` post-fix.
    public func resolveFont(_ spec: FontSpec) -> Font {
        if let provided = fontProvider?(spec) {
            return provided
        }
        return SwiftJsonUIConfiguration.defaultFont(for: spec)
    }

    /// Built-in `Font` resolution used when no `fontProvider` matches. Public
    /// so tests / apps can compare against the library default without
    /// reimplementing the matrix.
    public static func defaultFont(for spec: FontSpec) -> Font {
        let resolvedSize = spec.size ?? 17

        var font: Font
        if let family = spec.family {
            font = Font.custom(family, size: resolvedSize)
            if let weight = spec.weight {
                font = font.weight(weight)
            }
        } else {
            font = Font.system(size: resolvedSize, weight: spec.weight ?? .regular)
        }
