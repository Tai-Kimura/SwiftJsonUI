...
public extension UIColor {
    ...
    class func findColorByJSON(attr:JSON) -> UIColor? {
        // 先に関数で色を検索（@color/primary などの関数形式を処理）
        if let closure = SJUIViewCreator.findColorFunc {
            if let background = attr.string, let color = closure(background) {
                return color
            } else if let background = attr.int, let color = closure(background) {
                return color
            }
        }
        // 関数で解決できなかった場合、hex文字列として処理
