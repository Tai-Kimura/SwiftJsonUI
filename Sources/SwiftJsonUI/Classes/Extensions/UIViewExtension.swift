...
var BorderShapeLayerKey: UInt8 = 11
var BorderStyleKey: UInt8 = 12
var BorderColorKey: UInt8 = 13
var BorderWidthKey: UInt8 = 14
...
public extension UIView {
    ...
    var borderShapeLayer: CAShapeLayer? {
        get {
            objc_getAssociatedObject(self, &BorderShapeLayerKey) as? CAShapeLayer
        }
        set {
            objc_setAssociatedObject(self, &BorderShapeLayerKey, newValue, .OBJC_ASSOCIATION_RETAIN)
        }
    }

    var storedBorderStyle: String? {
        get {
            objc_getAssociatedObject(self, &BorderStyleKey) as? String
        }
        set {
            objc_setAssociatedObject(self, &BorderStyleKey, newValue, .OBJC_ASSOCIATION_RETAIN)
        }
    }

    var storedBorderColor: UIColor? {
        get {
            objc_getAssociatedObject(self, &BorderColorKey) as? UIColor
        }
        set {
            objc_setAssociatedObject(self, &BorderColorKey, newValue, .OBJC_ASSOCIATION_RETAIN)
        }
    }

    var storedBorderWidth: CGFloat {
        get {
            (objc_getAssociatedObject(self, &BorderWidthKey) as? CGFloat) ?? 0
        }
        set {
            objc_setAssociatedObject(self, &BorderWidthKey, newValue, .OBJC_ASSOCIATION_RETAIN)
        }
    }

    /// Apply border with style (solid, dashed, dotted)
    func applyBorder(color: UIColor, width: CGFloat, style: String, cornerRadius: CGFloat = 0) {
        storedBorderColor = color
        storedBorderWidth = width
        storedBorderStyle = style

        // Remove existing shape layer if any
        borderShapeLayer?.removeFromSuperlayer()
        borderShapeLayer = nil

        // Clear layer border (used for solid)
        layer.borderWidth = 0
        layer.borderColor = nil

        if style == "solid" {
            // Use standard layer border for solid
            layer.borderColor = color.cgColor
            layer.borderWidth = width
        } else {
            // Use CAShapeLayer for dashed/dotted
            ...
            if style == "dashed" {
                shapeLayer.lineDashPattern = [NSNumber(value: 6), NSNumber(value: 3)]
            } else if style == "dotted" {
                shapeLayer.lineDashPattern = [NSNumber(value: Float(width)), NSNumber(value: Float(width * 2))]
                shapeLayer.lineCap = .round
            }

            // Set path based on corner radius
    ...
    /// Update border style dynamically (for binding)
    func updateBorderStyle(_ style: String) {
        guard let color = storedBorderColor else { return }
        let width = storedBorderWidth
        let cornerRadius = layer.cornerRadius
        applyBorder(color: color, width: width, style: style, cornerRadius: cornerRadius)
    }

    /// Update border shape layer path when bounds change
    func updateBorderShapeLayerPath() {
        guard let shapeLayer = borderShapeLayer else { return }
        let cornerRadius = layer.cornerRadius
        let path = UIBezierPath(roundedRect: bounds, cornerRadius: cornerRadius)
        shapeLayer.path = path.cgPath
        shapeLayer.frame = bounds
