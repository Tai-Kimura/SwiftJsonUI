...
public class UILayoutConstraintInfo {
    ...
    // RTL-aware padding (original values before safe area adjustment)
    public var originalPaddingStart:CGFloat!
    public var originalPaddingEnd:CGFloat!
    ...
    // RTL-aware padding properties (computed based on layout direction)
    public var paddingStart: CGFloat! {
        get {
            // In LTR: start = left, In RTL: start = right
            if UIApplication.shared.userInterfaceLayoutDirection == .rightToLeft {
                return originalPaddingEnd ?? paddingRight
            } else {
                return originalPaddingStart ?? paddingLeft
            }
        }
        set {
            originalPaddingStart = newValue
        }
    }
    ...
    public var paddingEnd: CGFloat! {
        ...
        set {
            originalPaddingEnd = newValue
        }
    }
    public var leftMargin:CGFloat!
    public var rightMargin:CGFloat!
    // RTL-aware margins
    public var startMargin:CGFloat!
    public var endMargin:CGFloat!
    ...
    private  func inheritGravityFrom(superview: UIView?) {
        guard let layoutGravities = superview?.constraintInfo?.gravities else { return }
        let orientation = (superview as? SJUIView)?.orientation
        ...
        if let orientation = orientation {
            ...
            // No orientation (ZStack-like) - apply both axes
            if self.centerHorizontal == nil && self.alignLeft == nil && self.alignRight == nil {
                self.centerHorizontal = layoutGravities[.centerHorizontal]
            }
            if self.centerVertical == nil && self.alignTop == nil && self.alignBottom == nil {
    ...
    public class func paddingsFrom(attr: JSON) -> [CGFloat?] {
        ...
        // Support "padding" or "paddings" as single value (all sides)
        let singlePadding = attr["padding"].cgFloat ?? attr["paddings"].cgFloat
        if let singlePadding = singlePadding, attr["paddings"].arrayObject == nil {
            let top = attr["paddingTop"].cgFloat ?? attr["topPadding"].cgFloat ?? singlePadding
            let left = attr["paddingStart"].cgFloat ?? attr["paddingLeft"].cgFloat ?? attr["leftPadding"].cgFloat ?? singlePadding
            let bottom = attr["paddingBottom"].cgFloat ?? attr["bottomPadding"].cgFloat ?? singlePadding
            let right = attr["paddingEnd"].cgFloat ?? attr["paddingRight"].cgFloat ?? attr["rightPadding"].cgFloat ?? singlePadding
            ...
            case 0:
                ...
                viewPaddings = [
                    attr["paddingTop"].cgFloat,
                    attr["paddingStart"].cgFloat ?? attr["paddingLeft"].cgFloat,
                    attr["paddingBottom"].cgFloat,
                    attr["paddingEnd"].cgFloat ?? attr["paddingRight"].cgFloat
                ]
            case 1:
                viewPaddings = [
                    attr["paddingTop"].cgFloat ?? paddings[0],
                    attr["paddingStart"].cgFloat ?? attr["paddingLeft"].cgFloat ?? paddings[0],
                    attr["paddingBottom"].cgFloat ?? paddings[0],
                    attr["paddingEnd"].cgFloat ?? attr["paddingRight"].cgFloat ?? paddings[0]
                ]
            case 2:
                viewPaddings = [
                    attr["paddingTop"].cgFloat ?? paddings[0],
                    attr["paddingStart"].cgFloat ?? attr["paddingLeft"].cgFloat ?? paddings[1],
                    attr["paddingBottom"].cgFloat ?? paddings[0],
                    attr["paddingEnd"].cgFloat ?? attr["paddingRight"].cgFloat ?? paddings[1]
                ]
            case 3:
                viewPaddings = [
                    attr["paddingTop"].cgFloat ?? paddings[0],
                    attr["paddingStart"].cgFloat ?? attr["paddingLeft"].cgFloat ?? paddings[1],
                    attr["paddingBottom"].cgFloat ?? paddings[2],
                    attr["paddingEnd"].cgFloat ?? attr["paddingRight"].cgFloat ?? paddings[1]
                ]
            default:
                viewPaddings = [
                    ...
                    attr["paddingStart"].cgFloat ?? attr["paddingLeft"].cgFloat ?? paddings[1],
                    attr["paddingBottom"].cgFloat ?? paddings[2],
                    attr["paddingEnd"].cgFloat ?? attr["paddingRight"].cgFloat ?? paddings[3]
                ]
            }
        } else {
            // RTL-aware: paddingStart/paddingEnd take precedence over paddingLeft/paddingRight
            // Also support leftPadding/rightPadding aliases
            viewPaddings = [
                attr["paddingTop"].cgFloat ?? attr["topPadding"].cgFloat,
                attr["paddingStart"].cgFloat ?? attr["paddingLeft"].cgFloat ?? attr["leftPadding"].cgFloat,
                attr["paddingBottom"].cgFloat ?? attr["bottomPadding"].cgFloat,
                attr["paddingEnd"].cgFloat ?? attr["paddingRight"].cgFloat ?? attr["rightPadding"].cgFloat
            ]
        }
        return viewPaddings
    }
    ...
    public class func marginsFrom(attr: JSON) -> [CGFloat?] {
        ...
        if let margins = attr["margins"].arrayObject as? [CGFloat] {
            ...
            case 0:
                ...
                viewMargins = [
                    attr["topMargin"].cgFloat,
                    attr["startMargin"].cgFloat ?? attr["leftMargin"].cgFloat,
                    attr["bottomMargin"].cgFloat,
                    attr["endMargin"].cgFloat ?? attr["rightMargin"].cgFloat
                ]
            case 1:
                viewMargins = [
                    attr["topMargin"].cgFloat ?? margins[0],
                    attr["startMargin"].cgFloat ?? attr["leftMargin"].cgFloat ?? margins[0],
                    attr["bottomMargin"].cgFloat ?? margins[0],
                    attr["endMargin"].cgFloat ?? attr["rightMargin"].cgFloat ?? margins[0]
                ]
            case 2:
                viewMargins = [
                    attr["topMargin"].cgFloat ?? margins[0],
                    attr["startMargin"].cgFloat ?? attr["leftMargin"].cgFloat ?? margins[1],
                    attr["bottomMargin"].cgFloat ?? margins[0],
                    attr["endMargin"].cgFloat ?? attr["rightMargin"].cgFloat ?? margins[1]
                ]
            case 3:
                viewMargins = [
                    attr["topMargin"].cgFloat ?? margins[0],
                    attr["startMargin"].cgFloat ?? attr["leftMargin"].cgFloat ?? margins[1],
                    attr["bottomMargin"].cgFloat ?? margins[2],
                    attr["endMargin"].cgFloat ?? attr["rightMargin"].cgFloat ?? margins[1]
                ]
            default:
                viewMargins = [
                    ...
                    attr["startMargin"].cgFloat ?? attr["leftMargin"].cgFloat ?? margins[1],
                    attr["bottomMargin"].cgFloat ?? margins[2],
                    attr["endMargin"].cgFloat ?? attr["rightMargin"].cgFloat ?? margins[3]
                ]
            }
        } else {
            // RTL-aware: startMargin/endMargin take precedence over leftMargin/rightMargin
