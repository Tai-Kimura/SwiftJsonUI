...
open class SJUIButton: UIButton {
    ...
    open class func createFromJSON(attr: JSON, target: Any, views: inout [String: UIView]) -> SJUIButton {
        ...
        if let fontFamily = attr["fontFamily"].string {
            let weight = SJUILabel.fontWeight(from: attr["font"].string)
            font = UIFont(name: fontFamily, size: size) ?? .systemFont(ofSize: size, weight: weight)
        } else {
        ...
        // Button internal padding (leftPadding, rightPadding, paddingTop, paddingBottom, paddings)
        let lp = attr["leftPadding"].cgFloat ?? 0
        let rp = attr["rightPadding"].cgFloat ?? 0
