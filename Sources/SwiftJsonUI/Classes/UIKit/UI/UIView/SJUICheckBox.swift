...
open class SJUICheckBox: UIButton {
    ...
    open class func createFromJSON(attr: JSON, target: Any, views: inout [String: UIView]) -> SJUICheckBox {
        let labelKey = attr["label"].stringValue
        Logger.debug("[SJUICheckBox] Looking for label with key: '\(labelKey)'")
        Logger.debug("[SJUICheckBox] Available views keys: \(views.keys.sorted())")
        let labelView = views[labelKey]
        Logger.debug("[SJUICheckBox] Found label view: \(labelView != nil ? "YES" : "NO")")
        let c = viewClass.init(withLabel: labelView, imagePath: attr["src"].string, onImagePath: attr["onSrc"].string)
