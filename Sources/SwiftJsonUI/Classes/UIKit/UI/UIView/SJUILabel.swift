...
open class SJUILabel: UILabel {
    ...
    /// Storage for closure-based onclick handlers, keyed by partial attribute index
    /// These are preserved across applyAttributedText calls
    private var partialAttributeOnClickHandlers: [Int: (UITapGestureRecognizer) -> Void] = [:]

    /// Set closure-based onclick handler for a partial attribute at the given index
    /// This is used by binding to dynamically set onclick callbacks
    /// - Parameters:
    ///   - index: The index of the partial attribute (partialAttributes array index)
    ///   - handler: The closure to call when the partial attribute is tapped
    public func setPartialAttributeOnClick(at index: Int, handler: ((UITapGestureRecognizer) -> Void)?) {
        if let handler = handler {
            partialAttributeOnClickHandlers[index] = handler
        } else {
            partialAttributeOnClickHandlers.removeValue(forKey: index)
        }
        // Apply to linkedRanges if already populated
        // Find linkedRange by partialAttributeIndex, not by array index
        for (linkedRangeIndex, linkedRange) in linkedRanges.enumerated() {
            if let partialAttrIndex = linkedRange["partialAttributeIndex"] as? Int, partialAttrIndex == index {
                if let handler = handler {
                    linkedRanges[linkedRangeIndex]["onclickClosure"] = handler
                } else {
                    linkedRanges[linkedRangeIndex].removeValue(forKey: "onclickClosure")
                }
                break
            }
        }
    ...
    /// Apply stored onclick handlers to linkedRanges after they are rebuilt
    /// Uses partialAttributeIndex stored in linkedRanges to match with setPartialAttributeOnClick index
    private func applyStoredOnClickHandlers() {
        for (linkedRangeIndex, linkedRange) in linkedRanges.enumerated() {
            if let partialAttrIndex = linkedRange["partialAttributeIndex"] as? Int,
               let handler = partialAttributeOnClickHandlers[partialAttrIndex] {
                linkedRanges[linkedRangeIndex]["onclickClosure"] = handler
            }
        }
    }
    ...
    open func applyAttributedText(_ text: String!) {
        ...
        if let hint = hint, let hintAttributes = hintAttributes, string.isEmpty {
            ...
            if let partialAttributes = self.partialAttributesJSON {
                ...
                // Apply stored onclick handlers after linkedRanges are rebuilt
                applyStoredOnClickHandlers()
    ...
    @objc open func onLinkTap(_ sender: UITapGestureRecognizer) {
        ...
        guard let characterIndex = self.characterIndexAtPoint(location) else { return }

        // Handle partialAttributes onclick
        for linkedRange in linkedRanges {
            guard let start = linkedRange["start"] as? Int,
                  let end = linkedRange["end"] as? Int,
                  characterIndex >= start && characterIndex <= end else { continue }

            // Support closure-based onclick (from binding)
            if let onclickClosure = linkedRange["onclickClosure"] as? ((UITapGestureRecognizer) -> Void) {
                DispatchQueue.main.async {
                    onclickClosure(sender)
                }
                break
            }

            // Support string-based onclick (Selector name)
            if let onclick = linkedRange["onclick"] as? String, let linkHandleDelegate = self.linkHandleDelegate {
                DispatchQueue.main.async {
        ...
        // Handle linkable URLs (detected by NSDataDetector)
        if linkable, let input = self.attributedText?.string {
            let detector = try! NSDataDetector(types: NSTextCheckingResult.CheckingType.link.rawValue)
            let matches = detector.matches(in: input, options: [], range: NSRange(location: 0, length: input.utf16.count))
            for match in matches {
                if match.range.location <= characterIndex && match.range.location + match.range.length >= characterIndex {
                    if let url = URL(string: (input as NSString).substring(with: match.range)),
    ...
    public class func createFromJSON(attr: JSON, target: Any, views: inout [String: UIView]) -> SJUILabel {
        ...
        if let edgeInsetStr = attr["edgeInset"].string {
            ...
            edgeInsets = insets
        } else if let paddings = attr["paddings"].arrayObject as? [CGFloat] {
            // Fall back to paddings for Label internal padding
            edgeInsets = paddings
        } else if let singlePadding = attr["paddings"].cgFloat ?? attr["padding"].cgFloat {
            edgeInsets = [singlePadding]
        }
        ...
        let font: UIFont
        if let fontFamily = attr["fontFamily"].string {
            let weight = Self.fontWeight(from: attr["font"].string)
            if let customFont = UIFont(name: fontFamily, size: size) {
                if let descriptor = customFont.fontDescriptor.withSymbolicTraits(Self.symbolicTraits(for: weight)) {
            ...
            font = UIFont(name: name, size: size) ?? (name == "bold" ? UIFont.boldSystemFont(ofSize: size) : UIFont.systemFont(ofSize: size))
        }
        ...
        if let onclick = attr["onclick"].string, !onclick.hasPrefix("@{") {
    ...
    static func fontWeight(from name: String?) -> UIFont.Weight {
